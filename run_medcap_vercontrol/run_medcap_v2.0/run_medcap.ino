...
int sendSysTime = 999;
int sysCompThresh = 120;
int sendDiaTime = 999;
int diaCompThresh = 300;
int sendMaxima = 999;
...
int accelPrevAmt = 20;
int accelPrevPos = 0;
float* xprev = new float[accelPrevAmt];
float* yprev = new float[accelPrevAmt];
float* zprev = new float[accelPrevAmt];
float diffThresh = 0.075;
bool sendData = true;
...
//////THERMOMETER///////////////////////////////
  ...
  */
  ...
  if (accelPrevPos == accelPrevAmt){
    accelPrevPos = 0;
    sendData = same(xprev) && same(yprev) && same(zprev);
  }

  xprev[accelPrevPos] = accel_x;
  yprev[accelPrevPos] = accel_y;
  zprev[accelPrevPos] = accel_z;
 ...
 //BT////////
  ...
  if (sysTime/1000 > sysCompThresh){
     sendSysTime = sysTime/1000;
  }
  if (diaTime/1000 > diaCompThresh){
     sendDiaTime = diaTime/1000;
  }
  if (maxima > 512){
     sendMaxima = maxima;
  }
  ...
  if (sendData){
    sprintf(buf, "%i,%i,%i,%i",round(sendSysTime),round(sendDiaTime),round(sendMaxima),round(coreTemp*10));
  }else{
    sprintf(buf, "%s","!!!!NO DATA!!!!");
  }
...
bool same(float *arr){
  ...
  for (int i=0; i<accelPrevAmt-1; i++){
    if (abs(arr[i+1]-arr[i]) > diffThresh) return false;
  }
